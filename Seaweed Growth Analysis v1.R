...
Basic.Visualizing<-function(dataset){
  ...
  print(summary(data))
  glimpse(data)
  p<- data %>% ggplot(mapping=aes(x = Day, y = Mass, colour = ID, text = paste("ID: ", ID)))+
...
Calc.Doubling.Rate<-function(dataset){
  ...
  data<- data %>% mutate(k = (Mass - prev.Mass)/(Day-prev.Day))
  data<- data %>% mutate(percent.k = (k/prev.Mass)*100)
    ...
    graph.1<-ggplot(data, mapping = aes(x =Days, y = k, colour = Identifier))+
      geom_point(alpha = 0.8, size = 2)+
      geom_line(alpha = 0.8)+
      ...
      labs(x="Days", y = "Growth Rate(k), cm/day", title="Growth Rate showing mean growth rate")+
      theme(plot.title = element_text(hjust = 0.5))
    print(graph.1)
   
    
    graph.3<-ggplot(data, mapping = aes(x = Days, y = percent.k, colour = Identifier))+
      geom_point(alpha = 0.8, size = 2)+
      geom_line(alpha = 0.8)+
    ...
    graph.4<-data %>% ggplot(mapping = aes(y = k, colour = Identifiers))+
      geom_boxplot()+
      labs(title = "Box plot of growth rate")+
      theme(plot.title = element_text(hjust = 0.5))
    print(graph.4)  
    
    graph.5<-data %>% ggplot(mapping = aes(y = percent.k, colour = Identifier))+
      geom_boxplot()+
      labs(title = "Box plot of percentage growth rate")+
      theme(plot.title = element_text(hjust = 0.5))
    print(graph.5)
    
    graph6<-data |> ggplot(mapping = aes(x = k, colour = Identifier))+
      geom_density()+
